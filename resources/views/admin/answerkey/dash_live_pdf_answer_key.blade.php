...
$date=date('Y-m-d');
	...
	$where1=['student_id'=>$stid,'mcq_question_paper_id'=>$qpid];  
	...
	$str=['mcq_question_paper_id'=>$qpid,'student_id'=>$stid];
 ...
 if(!empty($qpaper))
 {
	 ...
	 $qpaper_type=$qpaper->question_paper_type;
 }
 else
 {
	 $qpname=""; 
...
<?php
  ...
  else
  {
	  if($qpaper_type==2)
	  {
		  ...
		  foreach ($mt_results as $key => $value) 
			{
