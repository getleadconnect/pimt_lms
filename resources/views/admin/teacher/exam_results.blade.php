...
<style>
    ...
    .er-panel-title { font-size: 1rem; font-weight: 500; color: #0f172a; margin: 0 0 15px 0; }
    ...
    .er-filter .fld-course { width: 400px; max-width: 100%; }
    .er-filter .fld-paper  { width: 320px; max-width: 100%; }
    @media (max-width: 480px) {
        .er-filter .fld-course, .er-filter .fld-paper { width: 100%; }
    }
    .er-filter label {
        display: block; font-size: 0.78rem; font-weight: 600;
        color: #334155; margin: 0 0 6px;
    }
    .er-filter .form-control {
        border: 1px solid #e5e7eb !important;
        background: #f8fafc !important;
        border-radius: 10px !important;
        padding: 9px 12px !important; font-size: 0.92rem;
    }
    .er-filter .form-control:focus {
    ...
    .er-btn.primary { background: linear-gradient(135deg, #bcd4f8 0%, #89b7fb 100%);
    color: #1a2232;}
    .er-btn.primary:hover { box-shadow: 0 6px 14px rgba(99,102,241,0.30); }
    .er-btn.ghost   { background: #fff; color: #475569; border: 1px solid #e2e8f0; }
    .er-btn.ghost:hover { background: #f1f5f9; color: #0f172a; }
    .er-btn.export {
        background: linear-gradient(135deg, #aacaf9 0%, #94bdfa 100%);
        color: #000000;
        border: 1px solid #94bdfa;
    }
    .er-btn.export:hover { background: linear-gradient(135deg, #c2d3ed 0%, #e2e7f7 100%); }
    .er-btn:disabled { opacity: 0.55; cursor: not-allowed; }

    /* Reused styles from attended_students for consistency */
    ...
    .ev-question {
        ...
        padding: 14px 16px; font-size: 0.94rem; color: #0f172a; line-height: 1.5;
        margin-bottom: 12px;
    }
    .ev-label {
    ...
    .ev-answer {
        ...
        padding: 12px 14px; font-size: 0.92rem; color: #334155;
    ...
    .rank-pill.rank-gold   { background: rgba(245,158,11,0.18); color: #b45309; border-color: rgba(245,158,11,0.35); }
    .rank-pill.rank-silver { background: rgba(148,163,184,0.18); color: #475569; border-color: rgba(148,163,184,0.40); }
    .rank-pill.rank-bronze { background: rgba(217,119,6,0.16); color: #92400e; border-color: rgba(217,119,6,0.30); }
    .rank-pill i { font-size: 0.78rem; }
</style>

<div class="er-wrap">
    <div class="er-panel">
        <div class="er-panel-head">
            <h3 class="er-panel-title">
                <i class="fa fa-bar-chart me-2" style="color:#7c3aed;"></i>Exam Results
            </h3>
        </div>
        ...
        <div class="er-filter">
            ...
            <div>
                <button type="button" id="btnGet" class="er-btn primary"><i class="fa fa-filter"></i> Get</button>
            </div>
            <div>
                <button type="button" id="btnClear" class="er-btn ghost"><i class="fa fa-eraser"></i> Clear</button>
            </div>
            <div style="margin-left:auto;">
                <button type="button" id="btnExport" class="er-btn export" title="Export filtered results to CSV">
...
{{-- ===== Evaluate Modal (same as attended_students) ===== --}}
...
$(function () {
    ...
    var table = $('#datatable').DataTable({
        ...
        ajax: {
            url: "{{ route('teacher.exam-results-data') }}",
            data: function (d) {
                d.question_paper_id = currentPaperId;
            }
        },
        columns: [
            ...
            { data: 'rank',         orderable: false, searchable: false, className: 'text-end' }
        ],
        language: {
            emptyTable: 'Select a course and a question paper, then click Get.'
        }
    });
    ...
    $('#flt_course').on('change', function () {
        ...
        if (!cid) {
            $pap.html('<option value="">Select course first…</option>');
            return;
        }
    ...
    // Get → load students for the selected paper
    function refreshExportState() {
        $('#btnExport').prop('disabled', !currentPaperId);
    }

    $('#btnGet').on('click', function () {
        var paperId = $('#flt_paper').val();
        if (!paperId) {
            alert('Please select a course and a question paper.');
            return;
        }
    ...
    // Clear → reset all controls and clear the table
    ...
    // Export CSV → download via the dedicated endpoint with the current paper id.
    $('#btnExport').on('click', function () {
        if (!currentPaperId) {
            alert('Please filter by a question paper before exporting.');
            return;
        }
        var url = "{{ route('teacher.exam-results-export') }}"
                + '?question_paper_id=' + encodeURIComponent(currentPaperId)
                + '&type=csv';
        window.location.href = url;
    });

    // Initial state — disabled until a paper is chosen.
    refreshExportState();

    /* ===== Evaluate modal — same flow as attended_students ===== */
    var modal     = null;
    var questions = [];
    var current   = 0;
    var visited   = new Set();
    ...
    // Delegated click for dynamically-rendered Evaluate buttons in DataTable
    $('#datatable').on('click', '.btn-evaluate', function () {
        openEvaluate(
            parseInt($(this).data('student-id'), 10),
    ...
    document.getElementById('ev-submit').addEventListener('click', function () {
        ...
        .then(function (res) {
            ...
            if (res.ok && res.body && res.body.status) {
                ...
                modal.hide();
                if (typeof toastr !== 'undefined') toastr.success(res.body.message); else alert(res.body.message);
            } else {
